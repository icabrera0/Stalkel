export function render(container, data, scenario, t, onCapture) {
  ...
  function showDetail(item, backFn) {
    ...
    // Large colored placeholder with emoji
    const photoPlaceholder = document.createElement('div');
    photoPlaceholder.style.cssText = `width:100%;aspect-ratio:1;background:${esc(item.imageColor || '#f0f0f0')};border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:80px;margin-bottom:16px;`;
    photoPlaceholder.textContent = item.imageEmoji || '📷';
    ...
    const sections = albumNames.map((albumName, idx) => {
      ...
      if (albumItems.length === 0) {
        ...
        albumItems.forEach(item => {
          ...
          cell.style.cssText = `background:${esc(item.imageColor || '#f0f0f0')};display:flex;flex-direction:column;align-items:center;justify-content:center;`;
          cell.innerHTML = `
            <div style="font-size:48px;margin-bottom:4px;">${item.imageEmoji || '📷'}</div>
            <div style="font-size:10px;color:#666;text-align:center;padding:4px;line-height:1.2;">${esc(item.caption || '')}</div>
          `;
